...
function handleOptions(options, ctx) {
    ...
    for (const directory of directories) {
        const { id, dirname, path: indexPath = `/${directory.id}/`, layout: indexLayout = 'IndexPost', frontmatter, itemLayout = 'Post', itemPermalink = '/:year/:month/:day/:slug', pagination = {
            lengthPerPage: 10,
        }, } = directory;
        if (!indexPath) {
            continue;
        ...
        paginations.push({
            ...
            ...util_1.resolvePaginationConfig(Classifier_1.ClassifierTypeEnum.Directory, pagination, indexPath, ctx),
            pid: id,
            id,
        });
    }
    for (const frontmatterPage of frontmatters) {
        const { id, keys, path: indexPath, layout: indexLayout, frontmatter, pagination = {
            lengthPerPage: 10,
        }, } = frontmatterPage;
        if (!indexPath) {
            continue;
